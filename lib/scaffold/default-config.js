...
/**
 * Will return the path and default gamblecore-node configuration. It will search for the
 * configuration file in the "~/.gamblecoin" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.gamblecoin');
  var defaultConfigFile = path.resolve(defaultPath, './gamblecore-node.json');
      ...
      servicesConfig: {
        bitcoind: {
          spawn: {
            ...
            exec: path.resolve(__dirname, '../../bin/gamblecoind')
          }
        }
      }
    };
