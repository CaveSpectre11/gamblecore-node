...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'gamblecore-node': 'git://github.com/GambleCoin-Project/gamblecore-node.git',
    // 'insight-gmcn-ui': 'git://github.com/GambleCoin-Project/insight-gmcn-ui.git',
    'insight-gmcn-api': 'git://github.com/GambleCoin-Project/insight-gmcn-api.git'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/gamblecore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the gamblecore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the gamblecore-node directory and configuration
